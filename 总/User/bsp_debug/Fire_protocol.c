...
void set_computer_value(uint8_t cmd, uint8_t ch, void *data, uint8_t num)
{
    ...
    usart1_send((uint8_t *)&set_packet, sizeof(set_packet) - 1); // 发送数据头
    usart1_send((uint8_t *)data, num);                           // 发送参数
    usart1_send((uint8_t *)&sum, sizeof(sum));                   // 发送校验和
    // usart0_send((uint8_t *)&set_packet, sizeof(set_packet) - 1); // 发送数据头
    // usart0_send((uint8_t *)data, num);                           // 发送参数
    // usart0_send((uint8_t *)&sum, sizeof(sum));                   // 发送校验和
}

void usart0_send(uint8_t *data, uint8_t len)
{
    uint8_t i;
    for (i = 0; i < len; i++)
    {
